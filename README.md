...
4. Create a `.env` file at `/client` folder.
5. Add these variables in `.env`:
    ```
    APP_ENV=development
    APP_URL=http://template-app.test
    ```
6. Run the following commands in root folder.
    ```
    php artisan key:generate
    php artisan migrate:fresh
    php artisan storage:link
    ```
7. Start the development server from `/client` folder with command:
    ```
    npm run dev
    ```
